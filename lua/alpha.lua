...
local function enable_alpha(opts)
    ...
    vim.cmd([[
        noautocmd silent! setlocal bufhidden=wipe nobuflisted colorcolumn= foldcolumn=0 matchpairs= nocursorcolumn nocursorline nolist nonumber norelativenumber nospell noswapfile signcolumn=no synmaxcol& buftype=nofile filetype=alpha nowrap
...
local function start(on_vimenter, opts)
    ...
    local function draw()
        ...
        for k in ipairs(cursor_jumps_press) do
