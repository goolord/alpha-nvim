...
local function utf8_char_byte_len(char)
    local b = char:byte(1)
    if not b then
        return 0
    end
...
local function display_col_to_byte_index(line_text, target_col)
    local byte_idx = 1
    local width = 0

    while byte_idx <= #line_text do
        local char = line_text:sub(byte_idx, byte_idx)
        local len = utf8_char_byte_len(char)

        char = line_text:sub(byte_idx, byte_idx + len - 1)
        local char_width = vim.fn.strdisplaywidth(char)

        if width + char_width > target_col then
            break
        end
    ...
    return byte_idx - 1 -- Return 0-based index like Neovim expects
end
...
local function layout(conf, state)
    ...
    for _, hl_line in pairs(hl) do
        local bufnr = hl_line[1]
        local ns = hl_line[2]
        local hl_group = hl_line[3]
        local line = hl_line[4]
        local col_start_display = math.max(hl_line[5], 0)
        local col_end_display = hl_line[6]

        local line_text = vim.api.nvim_buf_get_lines(bufnr, line, line + 1, false)[1] or ""
        local col_start = display_col_to_byte_index(line_text, col_start_display)
        local col_end = display_col_to_byte_index(line_text, col_end_display)
